package assignment14.versions.version7;
...
public class RegexTester {
	...
	public RegexTester() {
		...
		// Bring in event informmation:
		
		String event = "Meet Martin on Wednesday 8th March at King's in the morning.";
		...
		System.out.println( event.replace(multiTestRegex(datePatterns, event), "")
							     .replace(multiTestRegex(timePatterns, event), "")
							     .replace(multiTestRegex( new ArrayList<String>(Arrays.asList(new String[]{"(A|a)t\\s[A-Za-z']+"})), event), "") // Because null affects replace.
							     .replace("at", "")
								 .replace("on", "")
								 .replace(".", "")
								 .trim()
